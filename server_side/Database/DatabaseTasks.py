...
class DatabaseTasks:
    ...
    def add_sprint_with_task(self, sprint_details, project_id):
        """
    ...
    def sprint_tasks(self, project_id, client_id):
        ...
        If no sprints exist, creates one with a default task.
        """
        cursor = self.conn.cursor()
        query = '''
        ...
        if not sprint_rows:
            # No sprints exist, create one with a default task
            sprint_id = self.add_sprint_with_task([None, "New Sprint (rename me)", "#aaf683"], project_id)
            self.add_empty_item(project_id, sprint_id, client_id, "New Task")
            return self.sprint_tasks(project_id, client_id)

        sprints_dict = {}

        for sprint_id, sprint_name, sprint_color in sprint_rows:
